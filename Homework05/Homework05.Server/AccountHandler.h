...
namespace AccountHandler
{
	...
	**/
	bool parseAccount(std::string accountInfo, std::unordered_map<std::string, bool>& accountStatus);
	...
	*		   false if an error occurs.
	**/
	bool readAccountFromFile(const std::string& filename, std::unordered_map<std::string, bool>& accountStatus);
}
