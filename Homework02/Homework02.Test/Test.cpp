...
int main(int argc, char* argv[])
{
	...
	{
		numOfThread[i] = i;
		_beginthreadex(0, 0, thread, (void*)&numOfThread[i], 0, 0); 
	}
...
unsigned __stdcall thread(void *param)
{
	...
	for (int i = 0; i < numConn; i++) {
		clients[i] = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
		if (connect(clients[i], (sockaddr *)&serverAddr, sizeof(serverAddr))) {
			printf("\n%d. Error: %d", numThread, WSAGetLastError());
			break;
		}
		numConnected++;
		Sleep(20);
		...
		if (ret < 0)
			printf("%d. recv() fail: %d.\n", numThread, WSAGetLastError());
		else {
			rBuff[ret] = 0;
			printf("%d. Concurent test: %s\n", numThread, rBuff);
			numSession++;
		}
	...
	for (int i = 0; i < numConn; i++) {
		...
		for (int k = 0; k < 5; k++) {
			...
			ret = recv(clients[i], rBuff, BUFF_SIZE, 0);
			if (ret < 0)
				printf("%d. recv() %d fail: %d.\n", numThread, k, WSAGetLastError());
			else {
				ok++;
			}
		}
		if (ok < 5) printf("%d. Concurency test 2 failed\n", numThread);
	}

	printf("\n%d. Number of success connection: %d", numThread, numConnected);
	printf("\n%d. Number of success session: %d\n", numThread, numSession);
